	...
	const bool debug)
{
  ...
  torch::Tensor maxPixelAttribute = torch::zeros({P, 1}, means3D.options());
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		maxPixelAttribute.contiguous().data<float>(),
		out_invdepthptr,
		out_depthptr,
		antialiasing,
  ...
  return std::make_tuple(rendered, out_color, radii, pixels, maxPixelAttribute, geomBuffer, binningBuffer, imgBuffer, out_invdepth, out_depth);
}
